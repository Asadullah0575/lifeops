...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-[11px] font-mono uppercase px-2.5 py-0.5 rounded-full border whitespace-nowrap ${
        styles[status] ?? "text-ink/50 bg-ink/5 border-ink/10"
      }`}
    >
      {status}
    </span>
        ...
        <div className="flex gap-1.5 p-1 rounded-xl bg-ink/5 border border-ink/10 self-start">
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-lg transition-colors font-medium ${
              filter === "all" ? "bg-paper text-ink shadow-sm" : "text-ink/60 hover:text-ink"
            }`}
          >
          ...
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-lg transition-colors font-medium ${
              filter === "action" ? "bg-paper text-ink shadow-sm" : "text-ink/60 hover:text-ink"
            }`}
          >
            Autonomous Actions
          </button>
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-lg transition-colors font-medium ${
              filter === "approval" ? "bg-paper text-ink shadow-sm" : "text-ink/60 hover:text-ink"
            }`}
          >
            Human Approvals
          </button>
          ...
          {filtered.map((item) => {
            ...
            return (
              <div key={`${item.kind}-${item.id}`} className="relative group">
                {/* Timeline node icon */}
                <div
                  className={`absolute -left-[31px] top-4 w-3.5 h-3.5 rounded-full border-2 bg-paper transition-transform group-hover:scale-125 ${
                    isApproval
